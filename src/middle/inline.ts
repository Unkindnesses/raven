...
import { MIR, WIntrinsic, WImport, asFunc } from '../frontend/modules'
import { Redirect, type Sig } from './abstract'
import { options } from '../utils/options'
import { CycleCache, Cache } from '../utils/cache'
...
function opcount(code: MIR): number {
  ...
  for (const [_, st] of code) {
    if (['tuple', 'ref', 'branch', 'cast', 'retain', 'release'].includes(st.expr.head)) {
