...
function compiled_wat(m)
  ...
  try
    ...
    return String(read(`wasm-tools print $tmp`))
  finally
    rm(tmp)
  end
end
...
@testset "binary" begin
  ...
  m = WebAssembly.Module(
    funcs = [Func(:add, [] => [i32], [], Block([Const(Int32(5))]), FuncInfo(tag"add"))])
  s = compiled_wat(m)
  @test occursin("func \$add (;0;) (type 0) (result i32)", s)
  @test occursin("i32.const 5", s)
  
  m = WebAssembly.Module(
    funcs = [Func(:add, [] => [f64], [], Block([Const(1.0)]), FuncInfo(tag"add"))])
  ...
  m = WebAssembly.Module(
    ...
    funcs = [Func(:add, [i32] => [i32], [], Block([Const(Int32(5))]), FuncInfo(tag"add"))])
